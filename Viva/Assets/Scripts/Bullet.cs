...
public class Bullet : MonoBehaviour {
	Vector3 dir, nextDir;
	float speed, speedDecay = 2f, minSpeed = 0.1f, startSpeed = 200;
	...
	{
		if (other.gameObject.CompareTag("Enemy") || other.gameObject.tag == "Wall")
        {
			speed = 0; //stop if it hits an enemy
		}
	}
	void CheckDisappear(){
	...
	IEnumerator Disappear(){
		...
		float disSpeed = 1.5f; //take 1/3 seconds to disappear
