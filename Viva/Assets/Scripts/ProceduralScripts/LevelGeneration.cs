...
public class LevelGeneration : MonoBehaviour {
    ...
    List<Room> roomsWithOneNeighbour = new List<Room>();
	...
	void CreateRooms(){
		...
		rooms[gridSizeX,gridSizeY] = new Room(Vector2.zero, 1); //creates start room in center of scene of type 1 (starting room type)
		...
		for (int i =0; i < numberOfRooms -1; i++)
        {
			...
			//grab new possible position
			checkPos = NewPosition();
			//testing possible new position
			if (NumberOfNeighbors(checkPos, takenPositions) > 1 && Random.value > randomCompare){   //if room has 1 neighbour and random value
				int iterations = 0;
				do{
					checkPos = SelectiveNewPosition();
					iterations++;

                    //creating exit room beside room with one neighbour
                   
                        rooms[(int)checkPos.x + gridSizeX, (int)checkPos.y + gridSizeY] = new Room(checkPos, 2);
                       
                    
                } while(NumberOfNeighbors(checkPos, takenPositions) > 1 && iterations < 100); //while room has 1 neighbour and still iterating
                if (iterations >= 50)
            ...
            /*foreach (Room room in rooms)
            {
                ...
                {
                    Debug.Log("is my type shit being used");
                    roomsWithOneNeighbour.Insert(0, room);
                    room.type = 2;
                    break;
            ...
            //finalize position
            rooms[(int) checkPos.x + gridSizeX, (int) checkPos.y + gridSizeY] = new Room(checkPos, 0); //calculates offset of and adds to array while cresting room of type 0 (normal room type)
			takenPositions.Insert(0,checkPos); //adds taken room position to array
        }
	...
	void DrawMap(){
		foreach (Room room in rooms){
           ...
           /* for (int i =0; i < rooms.Length; i++)
                if (i == rooms.Length-1)
                {
                    Instantiate(tester, rooms[i].transform.position, Quaternion.identity);
                }*/
